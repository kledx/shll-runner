...
export class LLMBrain implements IBrain {
    ...
    private buildSystemPrompt(actions: IAction[]): string {
        ...
        return [
            ...
            "- IMPORTANT: If the 'Active Goals' section lists ANY goals, you MUST NOT set done=true.",
            "  Active goals mean you have ongoing work. Set done=false and continue monitoring.",
            "  The user's follow-up messages are NOT completion signals — they may be providing info you asked for.",
            "- ACTIVE MONITORING (user said 'watch', 'monitor', 'DCA', or similar):",
            "  · Price within 3% of trigger target: nextCheckMs=10000 (10s) — CRITICAL, must not miss dip",
            "  · Price within 10% of trigger target: nextCheckMs=30000 (30s) — close, check frequently",
            "  · Price far from trigger (>10%): nextCheckMs=120000 (2min) — relaxed monitoring",
            "  · High volatility (>5% price change /1h): nextCheckMs=10000 (10s) — market moving fast",
            "- POSITION MANAGEMENT (you hold a token being monitored):",
            "  · Price approaching stop-loss (<3%): nextCheckMs=5000 (5s) — urgent",
            "  · Price moving favorably: nextCheckMs=30000 (30s) — moderate",
            "  · Stable / sideways: nextCheckMs=60000 (1min) — relaxed",
            ...
            "- TRACK your goals: the Active Goals section shows your persistent objectives — use them as your memory",
            "- ADJUST your check interval based on how close conditions are to triggering action",
            "- COMPLETE goals: call manage_goal('complete', goalId) after fulfilling a goal",
            "",
