...
export class LLMBrain implements IBrain {
    ...
    private buildSystemPrompt(actions: IAction[]): string {
        ...
        const parts: string[] = [
            ...
            "  - Vault balance is literally 0 for the token you need → blocked: true",
            ...
            "- If a trade was blocked by safety policy (e.g. 'DEX not whitelisted', 'Token not in allowed list', 'exceeds limit'):",
            '  → This is a USER CONFIGURATION issue, not an authorization issue. Set blocked: true, blockReason: "Your trade was blocked by your safety policy settings. Check your Safety Config in the Console to update your allowed DEXes, tokens, or spending limits."',
            "- If a trade fails with 'Unauthorized' or 'operator expired' error specifically from on-chain execution:",
            ...
            "- Do NOT assume authorization expired for policy violations or other errors. Read the actual error message carefully.",
            "",
            "## Scheduling Control",
