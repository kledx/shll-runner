...
export class RunnerStore {
    ...
    async init(): Promise<void> {
        ...
        // P-2026-027: performance dashboard columns
        await this.pool.query(`
            ALTER TABLE runs
            ADD COLUMN IF NOT EXISTS gas_used TEXT NULL
        `);
        await this.pool.query(`
            ALTER TABLE runs
 ...
 */
    ...
    // ── P-2026-027: Performance Dashboard ─────────────────────────────────

    /**
     * Full performance data for a single agent token.
     * Includes aggregated metrics + daily PnL time series.
     */
    async getPerformance(tokenId: bigint): Promise<{
        totalRuns: number;
        successRuns: number;
        failedRuns: number;
        successRate: number;
        ...
        // Compute days active for avg daily runs
        ...
        // Compute max drawdown from daily cumulative PnL
    ...
    /**
     * Template-level performance aggregation.
     * Groups by brain_type (strategy template) across all tokens.
     */
